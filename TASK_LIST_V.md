...
where secrets come from, which Bitbucket/SharePoint hosts, the model backend, and the runtime tool.
...
- **Registry** (`publish_registry.py` push + `hydrate.py`/`generate.py` pull) uses **ambient git
  transport** — configure a git credential helper with the HTTP access token (V-04a).
- **Code source** (`clone.py`) uses the **auth seam** — `PDLC_AUTH_BITBUCKET` (or the JPMC
  secret-store backend) via `jpmc_adapters.auth` (V-04b).
- **SharePoint** (`ingest_sharepoint.py`) uses the **auth seam** for the Bearer token (V-08).
...
- [ ] **V-01 — Runtime tool in VS Code (the executor)**
  - **Depends on:** V-00.
  - **Reads:** `overlays/<tool>/launch.md`; `overlays/copilot/VDI_PREREQUISITES.md`; `docs/COPILOT_VDI_VALIDATION.md`.
  - **Do:** Install/enable the chosen runtime tool — **Claude Code** (terminal session) or **GitHub Copilot agent mode**. For Copilot, set the terminal command **allow-list at USER scope** (`github.copilot.chat.agent.terminal.allowList`) — provisioned centrally, never in the scaffold.
  - **Acceptance:** the tool loads custom agents/skills and can run a terminal command without per-command approval stalls (COPILOT_VDI_VALIDATION Checks 1–3 PASS).
  - **Proof:** the validation runbook outcome recorded (PASS / PARTIAL-tuned).

- [ ] **V-02 — Model backend (Bedrock) env**
  - **Depends on:** V-01.
  - **Reads:** `docs/TECH_SPEC.md` §7.3; `overlays/<tool>/launch.md` (the launch env contract).
  - **Do:** Set the model-routing env at launch: `CLAUDE_CODE_USE_BEDROCK=1`, AWS region, Bedrock model id (latest available Claude). Copilot uses its own model selection.
  - **Acceptance:** a trivial model call routes through Bedrock (no external endpoint); skills stay model-neutral.
  - **Proof:** a model_call telemetry event with the Bedrock model id after a smoke prompt.

---

## Phase V1 — Bitbucket repos + auth binding

- [ ] **V-03 — Create the two Bitbucket repos + an HTTP access token**
...
- [ ] **V-04a — Ambient git auth for the registry path**
  - **Depends on:** V-03.
...
- [ ] **V-04b — Bind the auth seam to the VDI secret (code + SharePoint)**
  - **Depends on:** V-03; `jpmc_adapters/auth.py` (TASK-052).
...
- [ ] **V-05 — Publish the registry to Bitbucket (repo #1)**
  - **Depends on:** V-00, V-04a.
...
- [ ] **V-06 — Verified hydrate from the live registry**
  - **Depends on:** V-05.
  ...
  - **Proof:** the generate descriptor showing `registry_sha_verified` == the V-05 SHA.

---

## Phase V3 — Code source live

- [ ] **V-07 — Push the Stratus C code + live-clone it through the seam**
  - **Depends on:** V-04b; `core/scripts/clone.py` (TASK-054).
...
- [ ] **V-08 — Implement `_download_pdf` for the JPMC SharePoint tenant**
  - **Depends on:** V-04b; `core/scripts/ingest_sharepoint.py` (TASK-055).
...
- [ ] **V-09 — Configure a run in the UI → Generate (G0)**
  - **Depends on:** V-02, V-06, V-07, V-08.
  - **Reads:** `docs/ACCEPTANCE.md` (the offline spine run); `TASK_LIST.md` TASK-056.
  - **Do:** In the React UI, configure: domain `payment_brand`; sources = the **SharePoint PDF** + the **Bitbucket code repo**; registry = the **Bitbucket registry URL** + the V-05 `registry_sha`; runtime tool. Click **Generate** → scaffold laid, stops at **G0**.
...
- [ ] **V-10 — Run the spine → accepted BRD + FRD**
  - **Depends on:** V-09.
...
- [ ] **V-11 — Reconcile the D5 `card_brand` / `message_format` `emitted_by` gap**
...
- [ ] **V-12 — (Optional, 5B-carried) thread the real runtime tool through G1/G2 telemetry**
...
> ✅ **VDI bring-up done = V-10 green** (a fresh operator completes a live UI → Generate → tool →
