...
`FR-DC-21 (S)` **Vocabulary adequacy signal (the coverage-floor twin for the *dictionary*).** Alongside the §10.1/§10.5 **containment** check (`usage ⊆ vocabulary`, a build-time hard gate against tag *invention*), the build MUST also surface an **adequacy** signal — whether the frozen vocabulary is *too small* for the artifacts actually present — because the model cannot invent a tag (`tags ⊆ vocabulary`), so an uncovered concept degrades to `tags: []` (for code) or no topics (for a doc) and is otherwise **silent**. (L1, in-slice — TASK-013) A **deterministic, model-free detector** computes `untagged_ratio` over both arms (code: `code_map` entries with empty `tags`; docs: manifest entries with no topics) and raises a `VOCAB_GAP_FLAG` when it exceeds a defined `adequacy_threshold` — never auto-grows the vocabulary, exactly as the coverage floor never auto-modifies the extractor (FR-DC-16). (L2, deferred to port — Phase 5) The model diagnosis of *which* concept warrants a new tag (working name `vocab_gap_assess`, scoped to the newly-introduced untagged delta), the human-gated vocabulary **amendment** (an addition; D5's set is never silently redefined), the `vocab_sha` version bump, and the re-tag pass are deferred — their first meaningful exercise is the real (VDI) corpus. *Why: an undersized vocabulary is the one failure mode containment cannot catch (it checks the opposite direction); turning the silent `tags:[]` gap into a trending, human-actionable flag is the vocabulary analog of the extractor coverage floor, and the deterministic detector earns its place in-slice because it is model-free and is the exact twin of a signal the MVP already builds.*
