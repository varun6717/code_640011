...
runs: once per Confluence source · the SOLE step of the `confluence` docs_pipeline lane (no pdf_extract / article_summarize)
---
...
through `pdf_extract` (already text) or `article_summarize` (a KB is not an article to summarize).
You tag; you do not summarize.
...
- **Never assign a tag outside your six.** The compliance-driven tags (`mandate`,
  ...
  passing does **not** earn `certification` — that tag is `article_summarize`'s, and a passing
...
- **Do not summarize.** You produce a short grounded `descriptor` line so the manifest entry is
  legible, but you do **not** write an `article_summarize`-style summary section.
...
- **`card_brand`** — which card brand(s) the page concerns. *(Shared with `article_summarize` /
  `code_map_build`.)*
...
- **`routing`** — transaction routing to brand handlers. *(Shared with `article_summarize` /
  `code_map_build`.)*
...
- Do not emit another skill's tags; do not write a summary section.
...
- Does not summarize (no `article_summarize`) — it tags the staged page and writes a descriptor only.
